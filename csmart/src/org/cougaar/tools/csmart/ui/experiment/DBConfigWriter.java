 ...
 **/
public class DBConfigWriter implements ConfigurationWriter {
  transient NodeComponent[] nodesToWrite;
  transient List components;
  ComponentData theSoc;

  public DBConfigWriter(List components, NodeComponent[] nodesToWrite, Experiment exp)
    throws SQLException, IOException
  {
    ...
    PopulateDb pdb = new PopulateDb("CSM", exp.getExperimentID(), exp.getTrialID());
    pdb.setDebug(true);
  ...
  private void addAgents(NodeComponent node, ComponentData nc) {
    ...
    for (int i = 0; i < agents.length; i++) {
      ...
      ac.setName(agents[i].getShortName());
