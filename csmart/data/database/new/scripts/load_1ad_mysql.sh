...
# Load the Configuration data contained in csmart/data/database/new/csv
...
mysql -u$1 -p$2 $3 < $COUGAAR_INSTALL_PATH/csmart/data/database/new/scripts/noncsmart-db.drop-mysql-tables.sql
echo "Creating tables in database."
mysql -u$1 -p$2 $3 < $COUGAAR_INSTALL_PATH/csmart/data/database/new/scripts/noncsmart-db.create-mysql-tables.sql
...
sed s/:cip/$(cat newcip.txt | sed 's/\//\\\//g')/ $COUGAAR_INSTALL_PATH/csmart/data/database/new/scripts/noncsmart-db.load-mysql-tables.sql > load_mysql_db_new.sql
rm newcip.txt
...
if [ ! -e "$COUGAAR_INSTALL_PATH/csmart/data/database/scripts/mysql/sedscr.sh" ]; then
    ...
    $COUGAAR_INSTALL_PATH/csmart/data/database/scripts/mysql/sedscr.sh $COUGAAR_INSTALL_PATH/csmart/data/database/new/csv/*.csv
fi
...
mysql -u$1 -p$2 $3 < $COUGAAR_INSTALL_PATH/csmart/data/database/new/scripts/noncsmart-db.create-mysql-indexes.sql

rm load_mysql_db_new.sql
rm -f load_mysql_db_new_orig.sql
rm $COUGAAR_INSTALL_PATH/csmart/data/database/new/csv/*.tmp
