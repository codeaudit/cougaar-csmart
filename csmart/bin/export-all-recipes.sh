...
if [ "x$4" = "x" ]; then
  RECIPES=`mysql -f -N -u $1 -p$2 $3 < ${COUGAAR_INSTALL_PATH}/csmart/bin/all-recipes.sql`
else
  RECIPES=`mysql -f -N -u $1 -p$2 -h $4 $3 < ${COUGAAR_INSTALL_PATH}/csmart/bin/all-recipes.sql`
fi

# rm recipes.txt
# echo $RECIPES > recipes.txt

rm ./export-all-recipes-from-$3.sh
